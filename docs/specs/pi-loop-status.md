...
- AC-S10: A user message whose first line is a fire header `[loop <name> #<n> <YYYY-MM-DD HH:mm>]` is displayed as the single heading line `## <name> #<n> · <YYYY-MM-DD HH:mm>` (level 2, which pi draws in the heading color with no marker); the prompt text after the header is not displayed. Assistant text, user text without that first line, and the header anywhere but the first line are returned untouched. The message sent through `sendUserMessage` is the plain bracket form plus the prompt (AC-1), and that is what the model sees.
