...
- AC-S10: A user message whose first line is a fire header `[loop <name> #<n> <YYYY-MM-DD HH:mm>]` is displayed with that line as the heading `### ↻ <name> #<n> · <YYYY-MM-DD HH:mm>` followed by a blank line; the rest of the message is unchanged. Assistant text, user text without that first line, and the header anywhere but the first line are returned untouched. The message sent through `sendUserMessage` is the plain bracket form (AC-1).
