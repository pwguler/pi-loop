...
## Acceptance criteria
- AC-1: `/loop <interval> <prompt>` creates a loop and fires it immediately as one normal trailing user message whose first line is `[loop <name> #1 <YYYY-MM-DD HH:mm>]`, followed by the prompt text. Interval syntax `5m`, `2h`, `1d`; minimum `1m`; below minimum or unparsable rejects with an error and creates nothing.
...
- AC-5: On resume, a loop whose due time has passed fires once at the first idle moment, then continues on cadence from that fire. Multiple missed ticks collapse into that one fire.
...
- AC-7: `/loop list` shows for each loop: name, interval, next due (local time), iteration count, status (`active`, `paused`, `owned by pid n`), last error if any. `/loop stop <name>` removes the loop. `/loop pause <name>` and `/loop resume <name>` toggle firing and keep the counter; resume counts the next due from the resume moment. Bare `/loop` prints the same as `/loop list`.
- AC-8: `--name <n>` sets the loop name; default is `loop-<k>` with the lowest free k. Creating a loop with an existing name rejects with an error.
